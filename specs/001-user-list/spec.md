...
2. **Given** the Users table is empty, **When** a user navigates to the users page, **Then** they see an empty table or appropriate message indicating no users exist
...
- **FR-005**: System MUST handle cases where the Users table is empty by displaying appropriate feedback to the user
...
- **User**: Represents a user in the system with attributes: username (text identifier, optional/nullable, must be unique when present) and email address (contact information, required, must be unique)
